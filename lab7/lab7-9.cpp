#include <iostream>
#include <iomanip>
using namespace std;

void findMaxSum(int[], int[][5], int, int);
void findMaxElm(int[], int[][5], int, int);
void findMaxVal(int[], int[][5], int, int);
const int SIZE = 10;

int main()
{
  const int NUM_ROWS = 6;
  const int NUM_COLS = 5;
  int result[SIZE] = {0};
  int numbers[NUM_ROWS] [NUM_COLS] = {{2, 7, 9, 6, 4},
                                      {6, 1, 8, 10, 4},
                                      {4, 3, 7, 2, 9},
                                      {9, 9, 0, 3, 1},
                                      {8, 8, 7, 8, 9},
                                      {1, 2, 1, 2, 3}};

  findMaxSum(result, numbers, NUM_ROWS, NUM_COLS);
  cout << "The row values are: ";
  for(int i = 0; i < NUM_ROWS; i++)
  {
    cout << setw(5) << result[i];
  }
  cout << endl;

  findMaxElm(result, numbers, NUM_ROWS, NUM_COLS);
  cout << "The Max values in the row are: \n";
  for(int i = 0; i < NUM_ROWS; i++)
  {
    cout << setw(5) << result[i] << endl;
  }

  findMaxVal(result, numbers, NUM_ROWS, NUM_COLS);
  cout << "The row that has the max value ";
  for(int i = 0; i < NUM_COLS; i++)
  {
    cout << setw(5) << result[i];
  }
  cout << endl;
  }

  // Return row with max summation value
  void findMaxSum(int result[], int num[][5], int rows, int cols) // Find the row summation values
  {
    int maxrowsum = 0, maxrow;
    for(int i = 0; i < rows; i++)
    {
      int rowsum = 0, max;
      for(int j = 0; j < cols; j++)
      {
        rowsum += num[i][j];
      }
      if ( maxrowsum < rowsum || i == 0)
      {
        maxrowsum = rowsum;
			  maxrow = i;
      }
    }
    for(int i=0; i<cols ; i++)
		  result[i] = num[maxrow][i];
  }


  // Find max elements from each row and return it
  void findMaxElm(int result[], int num[][5], int rows, int cols) 
  {
    for(int i = 0; i < rows; i++)
    {
      int max = 0;
      for(int j = 0; j < cols; j++)
      {
        if (num[i][j] > max)
          max = num[i][j];
      }
      result[i] = max;
    }
  }

  void findMaxVal (int result[], int num[][5], int rows, int cols) // find the max element, copy row
  {
    int max = 0, newmax = 0;
    int rowmax;
    for(int i = 0; i < rows; i++)
    {
      for(int j = 0; j < cols; j++)
      {
        if (num[i][j] > max)
          max = num[i][j];
      }
      if (max > newmax )
      {
        newmax = max;
        rowmax = i;
      }
    }
    for(int i = 0; i < cols ; i++)
		  result[i] = num[rowmax][i];
  }

  
  
  
  
  
